...
const DEFAULT_CODE: Record<Lang, string> = {
  html: SAMPLES['iPhone Home'].code,
  python: SAMPLES.Python.code,
  javascript: SAMPLES.JavaScript.code,
  dart: SAMPLES.Dart.code,
  typescript: 'console.log("Hello from TypeScript");\n',
}

export default function App() {
  const [codeByLang, setCodeByLang] = useState<Record<Lang, string>>(() => ({ ...DEFAULT_CODE }))
  ...
  useEffect(() => {
    ...
    term.writeln('Dart · Python · JS · HTML — each language is independent')
    term.writeln('')
  ...
  }, [code, language, isRunning, clearPreview])
  ...
  const changeSample = (name: string) => {
    ...
    if (s) {
      ...
      setLanguage(lang)
      clearPreview()
      setStatus('Sample loaded: ' + name)
      setActiveTab('editor')
    }
  }

  const fontSize = isMobile ? 13 : 14
  // Only show HTML preview when current language is HTML and we actually ran HTML
    ...
    return (
      <div className="fixed inset-0 z-[100] bg-black flex flex-col">
        <div className="shrink-0 flex items-center justify-between gap-2 px-3 py-2 bg-black/95 border-b border-white/10 safe-top">
          <button onClick={() => setShowIPhone(false)} className="h-9 px-4 rounded-full bg-white/15 text-white text-sm font-medium shrink-0">← Back</button>
          <span className="text-[11px] text-white/55 text-center flex-1 truncate">Passcode 000000 · swipe up = unlock</span>
          <button onClick={() => { const el = document.getElementById('iphone-frame') as HTMLIFrameElement | null; if (el) el.src = IPHONE_LIVE + '?t=' + Date.now() }} className="h-9 px-3 rounded-full bg-white/15 text-white text-xs shrink-0">Reboot</button>
        </div>
        <iframe id="iphone-frame" title="iPhone 17 Simulator" src={IPHONE_LIVE} className="flex-1 w-full border-0 bg-black" allow="camera; microphone; fullscreen; autoplay" sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-modals" />
      </div>
    )
  }
  ...
  const LangSelect = ({ className }: { className?: string }) => (
    <select className={className || 'bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-xs'} value={language} onChange={(e) => switchLanguage(e.target.value as Lang)}>
        ...
        <div className="flex items-center justify-between px-4 py-2.5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">Each language is independent · Preview only for HTML</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <div className="w-1/2 flex flex-col min-h-0">
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-[#070b14]">
            <div className="px-3 py-1.5 text-xs border-b border-slate-800 flex justify-between bg-[#0c1220] text-slate-400">
              <span className="text-slate-300 font-medium">iPhone 17 Preview {language !== 'html' ? '(HTML only)' : ''}</span>
